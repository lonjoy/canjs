	...
	can.extend(proto, {
		...
		_getInsertIndex: function (item) {
			...
			for (var i = 0; i < length; i++) {
				...
				// index isn't a candidate. If we let ignored this, it would
		...
		_getRelativeInsertIndex: function (item, currentIndex) {
			...
			// _getInsertIndex attempts to move the item as far back into
			// the list as possible. If in doing that it found a candidate
			// index before it got to the item's currentIndex go with that.
			if (naiveInsertIndex <= currentIndex) {
				return naiveInsertIndex;
			}

			// If a forward swap is suggested by _getInsertIndex, inspect
			// the items between the two indexes for items of the same value.
			// Otherwise, we may be needlessly leapfroging over same value
			// items to be naively positioned before an item with a greater
			// value. Notes:
			//   currentIndex + 1; Avoid evaluating (item) with itself
			//   i <= naiveInsertIndex; Only evaluate items after current and
			//     before suggested
			for (i = currentIndex + 1; i <= naiveInsertIndex; i++) {
